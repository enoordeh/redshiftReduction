...
function zcompute, objflux, objivar, starflux, starmask, $
 ...
 zans_fixed=zans_fixed, silent=silent
   ...
   for ilag=0L, nlag-1 do begin
      ...
      i2 = i1 + j2 - j1
      if (keyword_set(fixed_template)) then chi2arr[ilag] = computechi2( objflux_double[i1:i2], sqivar[i1:i2] * starmask[j1:j2], [starflux_double[j1:j2,*]],[fixed_template[i1:i2,*]], dof=dof) else chi2arr[ilag] = computechi2( objflux_double[i1:i2], sqivar[i1:i2] * starmask[j1:j2], starflux_double[j1:j2,*], dof=dof)
      dofarr[ilag] = dof
