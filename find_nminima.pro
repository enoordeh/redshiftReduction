...
; return without having defined perror. 
      ...
      if (coeff[0] GT 0) then begin
         print, "ybest is a maximum - ignore"
         errcode = -4L
      endif
...
;      !x.omargin = [10,3]
   ...
   while (npeak LT nfind) do begin
      ...
      junk = min(ycopy, imin)
      ; print, "imin = ", imin 
      ; print, "junk(min(chi2arr/dof)) = ", junk
      ; print, imin
...
; ???
         ...
         ; plot, [xplotval], [yplotval], psym=4
         if (errcode1 EQ 0) then color = 'green' $
          else color='red'
          print, "errcode 1 = ", errcode1
         ...
         djs_oplot, [xpeak1,xpeak1]+xerr1, !y.crange, linestyle=2, color=color
         ; oplot, [xplotfit], [yplotfit], color=color
         ; oplot, [xpeak1], [ypeak1], psym=2, symsize=2, color=color
         ; oplot, [xpeak1,xpeak1]-xerr1, !y.crange, linestyle=2, color=color
         ; oplot, [xpeak1,xpeak1]+xerr1, !y.crange, linestyle=2, color=color
      endif
      ...
      junk = min(abs(xvec - xvec[imin]), ixc) ; *Emil* doesn't ixc=imin?
      ; print, "ixc", ixc
      ; print, "imin", imin ; *Emil* Yup..
   ...
   if (keyword_set(doplot)) then begin
      ...
      for ipeak=0, npeak-1 do begin
         ...
         ; print, "xpeak[ipeak] ", xpeak[ipeak]
         ; print, "ypeak[ipeak] ", ypeak[ipeak]
         oplot, [xpeak[ipeak]], [ypeak[ipeak]], psym=2, color=color
      endfor
      !p = bangp
      !p.color = djs_icolor('default')
      ; !p.color = icolor('default')
   endif
