...
;
; CALLING SEQUENCE:
;   result = zfind_CV( objflux, objivar, hdr=hdr, $
...
;                   lambda. triggers fnosubthe mapping to loglambda
...
function zfind_CV, ss1d, eigenfile=eigenfile, eigendir=eigendir, $
                ...
                nosubtract=nosubtract, wvmin=wvmin, wvmax=wvmax, fname=fname, debug=debug, tclass=tclass, physcheck=physcheck, $
                _EXTRA=EXTRA

  common com_zfind_CV, starflux, starloglam0, stardloglam, $
    nstars, npoints, starflux_corrected, thisfile

;  common com_zfind, starflux, starloglam0, stardloglam, $
;    nstars, npoints, thisfile
...
; check if continuum subtraction is required.
  ...
  ; print, "nosubtract = ", nosubtract
  ;print, "starflux = ", starflux
  ;starflux_corrected = starflux
...
;;; FROM THE HEADER, GRAB THE WAVELENGTH INFORMATION.
    ...
    ; print, "here"
    starflux_corrected = starflux ; becky
...
;;; ACCORDINGLY ONLY KEEPING THE DESIRED TEMPLATE SPECTRA. 
  ...
  ; print, "(zfind.pro) columns: ", columns
...
if (keyword_set(debug)) then begin
  print, "************ PLOTTING CONTINUUM SUBTRACTED OBJECT SPECTRUM (AFTER TRIMMING) ************"
  lengthtest=n_elements(objflux)
  xaxistest=indgen(lengthtest)
  cgoplot, xaxistest, objflux, color=FSC_color('white')
  pause
endif
