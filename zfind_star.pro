...
; remove continuum from template at this level
    ...
    for i=0, nstars-1 do begin ;correct all stars
      ...
      if (keyword_set(debug)) then begin
        print, "************ PLOTTING RAW STAR TEMPLATE SPECTRUM ************"
        lengthtest=n_elements(starflux[*,i])
        xaxistest=indgen(lengthtest)
        cgplot, xaxistest, starflux_in[*,i], color=FSC_color('red'), XTicklen=1.0, YTicklen=1.0, AxisColor='white',$
      ...
      if (keyword_set(debug)) then begin
        print, "************ PLOTTING CONT SUB STAR TEMPLATE SPECTRUM ************"
        lengthtest=n_elements(starflux[*,i])
        xaxistest=indgen(lengthtest)
        cgoplot, xaxistest, starflux[*,i], color=FSC_color('blue')
        pause
      endif
